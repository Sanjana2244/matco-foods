    ...
    <script>
        $(document).ready(function() {
            var table = $('#dTable').DataTable({
                ...
                ajax: {
                    url: '{{ route('admin.sliders.getList', ['type' => $sliderType]) }}',
                    type: 'GET',
                    dataType: "JSON",
                ...
                columns: [{
                    ...
                    },
                    {
                        data: 'type',
                        render: function(full, type, data, meta) {
                            return `<div class="relative mrg-top-15"><span class="pdd-left-20">${data.type}</span></div>`;
                        }
                    },
                    {
                        data: 'Actions',
                        render: function(full, type, data, meta) {
                            return `<div class="mrg-top-10">
                                <a class="pl-3" href="/admin/sliders/${data.id}/edit"><i
                                        class="ti-pencil"></i></a>
                                <a class="pl-3 delete" href="/admin/sliders/${data.id}"><i
                                        class="ti-trash"></i></a>
                            </div>`;
                        }
                    },
            ...
            table.on('row-reorder', function(e, diff, edit) {
                for (var i = 0, ien = diff.length; i < ien; i++) {
                    if (diff[i].oldPosition != -1) {
                        fetch('{{ route('admin.sliders.reorder') }}?' + new URLSearchParams({
